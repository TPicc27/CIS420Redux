...
Skin:<select id="skin_select" name="skin" >
    <% For Each skin As String In [Enum].GetNames(GetType(DHTMLX.Scheduler.DHXScheduler.Skins)) %>
        <option value="<%= skin%>" id="<%= skin%>"><%= skin%></option>
    <% Next%> 
     </select>&nbsp;&nbsp;
Language:<select id="language_select" name="language">
	<% For Each locale As String In [Enum].GetNames(GetType(DHTMLX.Scheduler.SchedulerLocalization.Localizations))%>
...
<script type="text/javascript">
	...
	<% If (Not string.IsNullOrEmpty(Model.skin)) Then %>
		$("<%= Model.skin %>").selected = "selected";
	<% End if %>
	<% If (Not string.IsNullOrEmpty(Model.locale)) Then %>
		$("<%= Model.locale %>").selected = "selected";
	<% End if %>
</script>
